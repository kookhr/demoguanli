...
- **CORS 规避**: 使用 no-cors 模式绕过浏览器跨域限制
- **Mixed Content 处理**: 自动处理 HTTPS/HTTP 混合内容问题
- **双重策略**: no-cors 模式 + 图片探测法确保检测可靠性

### 检测模式
- **简化模式**（推荐）: 专注于服务可达性，快速简单
- **标准模式**: 使用原有的复杂检测策略
